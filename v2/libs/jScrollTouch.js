...
/**
  Modified by Ben Shapiro (bshapiro@morgridgeinstitute.org)
**/
...
(function($){
  $.fn.jScrollTouch = function () {
	...
	return this.each(function() {
		var cont = $(this);
		//alert("Hooking scroll up to id: "+cont.id)
		...
		if(isTouchScreen){
			...
			scrollbarV.css({'z-index':999,'display':'none','position':'absolute','width':'5px','height':scrollbarV_length+'px','left':cont.innerWidth()-7+'px','top':0,'background':'black','border':'1px white solid','-webkit-border-radius':'5px','opacity':'0.9'});
			
			var scrollbarH = $('<div></div>');
			scrollbarH.css({'z-index':999,'display':'none','position':'absolute','height':'5px','width':scrollbarH_length+'px','top':cont.innerHeight()-7+'px','left':0,'background':'black','border':'1px white solid','-webkit-border-radius':'5px','opacity':'0.9'});
		...
		cont.bind('mousedown touchstart',function(e){
			...
			cont.bind("jscrolltouch.unhook", function(ev){
				//alert("unhooking jscrolltouch from "+cont.id);
				cont.remove(scrollbarH);
				cont.remove(scrollbarV);
				cont.unbind('mousemove');
				cont.unbind('mousedown');
				cont.unbind('mouseup');
				cont.unbind('touchstart');
				cont.unbind('touchmove');
				cont.unbind('touchend');
				cont.unbind('jscrolltouch.unhook');
			});
		});
	});
}
